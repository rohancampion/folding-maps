...
export const advancedNewsDepth: DepthMap = {
  'ai-integration-gap': [
    ...
    [
      ...
      { text: 'Data lineage should remain visible from source record to generated output and accepted action. The workflow owner needs to know which version of a document, policy or account state supported the decision. Retention, deletion and access changes must propagate through indexes and caches. These controls carry implementation cost, yet they also make defects diagnosable and allow the service to recover without discarding every output produced during an incident window.' },
    ],
    [
  ...
  'automation-before-agents': [
    [
      { text: 'Following a representative item should record every wait state, retry and informal workaround. The apparent process may describe how work should move, while logs and interviews reveal how it actually moves. A state with no owner or completion evidence becomes an explicit design defect. Agent capability cannot repair that defect because the system still lacks a rule for what completion means.' },
    ...
    [
      ...
      { text: 'An operating owner must accept the residual risk and fund ongoing supervision. Technology can monitor traces and surface drift; it cannot decide how much service, financial or regulatory exposure the business is prepared to retain. The final approval should record scope, authority, fallback, incident ownership and the evidence date supporting the decision.' },
    ],
  ],
  'cold-chain-collaboration': [
    ...
    [
      { text: 'Observation mode needs a reconciliation method. Every service case should be matched to existing alerts and manual discoveries, while events found by only one route receive review. Precision alone is insufficient because a system can improve it by suppressing difficult alerts. Recall, alert burden, time to ownership and closure completeness must be considered together.' },
  ...
  'small-teams-ai-advantage': [
    ...
    [
      { text: 'Concentration risk should be visible in service ownership. If one colleague understands the workflow, model and recovery route, leave or absence can halt operation. Pairing, runbooks, access escrow and rehearsed manual fallback create resilience with limited overhead. External partners may provide specialist depth, while accountability for the operating outcome remains inside the firm.' },
  ...
  'measure-automation-value': [
    ...
    [
      { text: 'The value bridge needs an evidence owner at each transfer. Product telemetry can establish eligible use; workflow data can establish net effort; operations can confirm redeployment; finance can confirm cost avoided or contribution realised. No single system will prove the whole chain. Reconciliation cadence and confidence should be part of the benefits ledger.' },
  ...
  'legal-ai-source-grounded-work': [
    ...
    [
      ...
      { text: 'The accountable owner should review defects by proposition class and matter type. A recurring temporal-validity failure calls for source and metadata work; a recurring omission may require query decomposition or a narrower scope. This causal review prevents every failure from becoming a prompt change and directs investment to the actual weak control.' },
    ],
  ],
  'hospitality-ai-guest-recovery': [
    ...
    [
      { text: 'Connector ownership should be explicit for reservations, property management, maintenance, housekeeping and CRM. Each interface needs freshness, failure and reconciliation measures. When two systems disagree, the recovery case should show the conflict and source hierarchy. Hidden last-write-wins logic would convert an integration issue into a guest-facing commitment.' },
    ...
    [
      { text: 'Pilot selection should use a disruption with enough volume, clear ownership and a realistic route to outcome evidence. Room unavailability, maintenance displacement or missed amenity may qualify depending on the estate. The test should exclude cases whose remedy depends on systems or partners outside the current integration boundary.' },
      { text: 'Expansion should compare properties and channels without assuming one policy fits every operating context. Staffing, inventory, brand promise and local regulation can change feasibility and authority. Shared architecture can carry common identity, evidence and audit patterns while property policy remains versioned and locally accountable.' },
    ],
  ],
};
...
export const advancedCaseDepth: DepthMap = {
  'yacht-operations': [
    ...
    [
      ...
      { text: 'Ownership belongs to the next required action, not only the customer account. A commercial owner, delivery owner and reviewer may change as the item progresses. The workflow policy assigns a role, due condition and escalation route for each state. Absence or reassignment becomes an explicit event so active work cannot remain attached to a colleague who is unavailable.' },
    ],
    [
      { text: 'The read model used by staff should answer current state, last material event, next action, owner, open exception and source commitment without exposing the complexity of the event store. Management reporting derives from the same state model. This removes the need for a separate weekly status narrative and makes any discrepancy traceable to a source event or rule.' },
    ...
    [
      { text: 'The second-release decision should identify which exceptions dominate. Missing events may require source integration; abandoned records may indicate ownership or interface friction; low trust may trace to migration errors. The response should repair the weak mechanism before adding documents, reporting or AI assistance.' },
  ...
  'cold-chain': [
    ...
    [
      ...
      { text: 'Exception ownership requires severity, due condition, escalation and continuity across shifts. The queue should expose unassigned and ageing cases. Closure records inspection, corrective action, product decision and subsequent evidence. Reopening preserves the prior history when later readings or investigation change the conclusion.' },
    ],
    [
  ...
  'property-pipeline': [
    [
      ...
      { text: 'The baseline should trace a sample from initial enquiry to archive, recording stage, required evidence, dependency, owner, deadline, reversal and source systems consulted. Message count is excluded as a progress measure because activity can rise while a blocking condition remains untouched.' },
    ],
    [
      ...
      { text: 'Dependencies need first-class records. A search, valuation, approval, counterparty document or external decision can block several downstream actions. Each dependency has owner, source, expected date and effect. This allows the portfolio view to distinguish work awaiting internal action from work waiting on an external party.' },
    ],
    [
...
export const advancedNewsSections: Record<string, AddedSection[]> = {
  ...
  'automation-before-agents': [{
    ...
    purpose: 'Specify the controls needed when a model can observe and change live systems.',
  ...
  'cold-chain-collaboration': [{
    ...
    purpose: 'Define the ongoing engineering and governance needed after the exception route is built.',
    transition: 'An accountable exception case solves the decision path; sustained use depends on controlled telemetry, policy change and operating resilience.',
    paragraphs: [
      { text: 'The asset register should link sensors, gateways, network zones, calibration, maintenance owner and monitored product context. Architecture diagrams need effective dates and a reconciliation process with site changes. A definitive view does not stay definitive by itself, especially when maintenance teams replace equipment or move sensors during urgent work.' },
  ...
  'small-teams-ai-advantage': [{
    ...
    purpose: 'Show how a lean team can turn a focused release into reusable operating capability.',
    transition: 'Focus creates a credible first experiment; durable advantage depends on how its knowledge, controls and ownership transfer into normal work.',
    paragraphs: [
      { text: 'The delivery cell needs a sponsor, workflow owner, subject expert, technical owner and representative users. One person can hold more than one role in a small firm, but each decision right should remain explicit. This prevents speed from depending on an unnamed colleague who informally approves data, quality and service risk.' },
      { text: 'The cell should work in short releases with a current problem statement, acceptance evidence and stop condition. Demonstrations use representative items and expose open exceptions. A decision log records scope, source, risk, trade-off and owner. These light artifacts preserve learning without creating a separate transformation bureaucracy.' },
      ...
      { text: 'External specialists can supply architecture, evaluation and security depth that the firm does not need full-time. The operating team still owns purpose, customer promise and acceptance. Contracts should preserve access to configurations, tests, documentation and data decisions so supplier change does not erase the learning created by the project.' },
  ...
  'measure-automation-value': [{
    ...
    purpose: 'Specify how operations and finance maintain a credible value claim after launch.',
  ...
  'legal-ai-source-grounded-work': [{
    ...
    purpose: 'Extend proposition-level evidence into retention, reuse, disclosure and service operation.',
  ...
  'hospitality-ai-guest-recovery': [{
    ...
    paragraphs: [
      ...
      { text: 'Portfolio rollout should begin with comparable properties and one disruption class. Readiness covers source quality, operating ownership, delegated authority, fallback and measurement. Shared technology can scale, while each property retains an evidence-based decision about policy and feasible remedy. This prevents a central template from promising service the local operation cannot deliver.' },
    ],
  }],
};
  ...
  'yacht-operations': [
    {
      ...
      purpose: 'Specify how legacy customer and project data can enter the new record without creating false trust.',
      ...
      paragraphs: [
        { text: 'Migration begins with a source inventory covering inboxes, documents, spreadsheets and any existing customer tool. Each source receives an owner, extraction date, field map and quality profile. The team identifies which source can create or update customer, vessel, enquiry and project attributes. Conflicts remain visible until a defined owner resolves them.' },
    ...
    },
    {
      ...
      purpose: 'Define the live evidence, support model and decision required before broader automation.',
      ...
      paragraphs: [
        { text: 'Users should receive role-based views and a concise explanation of identity, events, state and ownership. Training uses representative enquiries and known exceptions. Support captures the record, state and action involved in each issue so the team can distinguish misunderstanding from data, policy or interface defects.' },
        { text: 'Acceptance sampling should reconstruct customer commitments from source event through current state and next action. The reviewer checks identity, chronology, owner, exception and approval. Status assembly time, systems consulted and overdue work provide coordination evidence. Material communication correction protects the service boundary.' },
        ...
        { text: 'Operational support needs ownership for source integrations, identity exceptions, workflow policy and user issues. Service hours and recovery expectations follow the customer promise. A small team can keep the model lean, but critical knowledge and access should not reside with one person.' },
        { text: 'The release review decides expand, adjust, hold or stop. Expansion requires trusted reconstruction, maintained ownership and acceptable support demand. Weak identity or migration evidence sends the design back to reconciliation. Low adoption sends it to workflow and interface review. Only then can document connection or bounded drafting enter scope.' },
      ],
    },
  ],
  'cold-chain': [
    {
      ...
      purpose: 'Define the technical evidence and boundaries needed before exception classification can be trusted.',
      ...
      paragraphs: [
        { text: 'The asset record should connect site, room, equipment, sensor, gateway, network segment, calibration, maintenance owner and product context. Each relationship has an effective period. Installation, replacement and relocation need controlled updates so the monitoring service does not apply the wrong context to a valid signal.' },
        ...
        { text: 'Connectivity monitoring covers heartbeat, queue depth, clock offset, certificate and gateway health. A data gap becomes an operating state with owner and fallback, not a blank chart. Site staff should know when the service has degraded and which existing check remains authoritative.' },
    ...
    },
    {
      ...
      purpose: 'Set the evidence needed to tune policy, preserve operator authority and decide production reliance.',
  ...
  'property-pipeline': [
    {
      ...
      purpose: 'Define the records, state and evidence needed to make a transaction stage reviewable.',
      ...
      paragraphs: [
        ...
        { text: 'Stage gates define minimum evidence, unresolved exceptions, approval and permitted next states. A person cannot create completion readiness by editing a label. An approved exception records scope, reason, owner and expiry. Reversal preserves the previous state and dependencies affected.' },
        { text: 'Dependencies carry type, owner, source, expected date, status and impact. Portfolio reporting can distinguish internal delay, external wait and missing evidence. This gives management an actionable risk view and avoids using message volume or anecdotal confidence as a proxy for progress.' },
    ...
    },
    {
      ...
      purpose: 'Define evaluation, adoption and expansion decisions for one transaction type.',
  ...
  'professional-services-intake': [
    {
      ...
      purpose: 'Define the taxonomy, provenance and processing boundaries behind a controlled matter candidate.',
      ...
      paragraphs: [
        { text: 'The intake schema should identify prospective client, related parties, contact, service need, jurisdiction, urgency, referral source and candidate matter. Each field has purpose, source, sensitivity and retention. Progressive capture requests information when it becomes necessary for the next gate.' },
    ...
    },
    {
      ...
      purpose: 'Set the quality, service and supervision evidence required for assisted intake.',
  ...
  'field-service-planning': [
    {
      ...
      purpose: 'Define the input, optimisation and recovery mechanics behind feasible dispatch options.',
    ...
    },
    {
      ...
      purpose: 'Define the comparison, acceptance and authority decision before live recommendations.',
...
export const advancedNewsExtensionSections: Record<string, AddedSection[]> = {
  'ai-integration-gap': [{
    ...
    purpose: 'Connect integrated workflow performance to a controlled portfolio decision.',
    ...
    paragraphs: [
      ...
      { text: 'Scaling should reuse data contracts, identity patterns, evaluation and incident routes while requiring a new workflow baseline and owner. Similar technology does not make two processes economically or operationally equivalent. Portfolio review should compare evidence confidence and management capacity alongside forecast return.' },
    ],
  }],
  'open-weight-price-war': [{
    heading: 'Deployment economics under change',
    purpose: 'Test how sourcing choices behave as demand, quality and supplier terms move.',
    ...
    paragraphs: [
      ...
      { text: 'Open deployment scenarios should include hardware failure, patching, scaling and staff availability. A partner-operated service can transfer some duties without removing buyer accountability for licence, data and acceptance. Contract and architecture should state that allocation clearly.' },
  ...
  'automation-before-agents': [{
    ...
    purpose: 'Define a release test that exposes rare failures and the cost of human rescue.',
    ...
    paragraphs: [
      ...
      { text: 'Consequence weighting belongs beside raw counts. One duplicate reminder and one duplicate payment are not comparable defects. The release review should identify blocking failure classes and acceptable residual error for lower-consequence work, with the owner signing the distinction.' },
  ...
  'cold-chain-collaboration': [{
    ...
    purpose: 'Connect technical control evidence to workload, reporting and multi-site decisions.',
    ...
    paragraphs: [
      ...
      { text: 'Reporting value comes from evidence already created during operation. If staff must re-enter decisions for audit or customer assurance, the system has not closed the route. Sampled reports should be reconstructed back to readings, context, policy, owner and corrective action.' },
      ...
      { text: 'Scale approval should require reliable local ownership, controlled architecture, calibrated policy, manageable workload and complete closure evidence. New asset classes or products trigger fresh hazard review. The service can reuse software while preserving site-level accountability.' },
  ...
  'small-teams-ai-advantage': [{
    ...
    purpose: 'Show how a lean organisation can fund, stop and reuse focused AI work.',
    ...
    paragraphs: [
      ...
      { text: 'A portfolio limit protects the organisation from many small pilots. New work enters only when an owner, baseline, evidence set and capacity budget exist. Existing work leaves when the outcome is immaterial, support is disproportionate or a stronger constraint appears.' },
      { text: 'Reusable assets lower the cost of later releases, but their maintenance also needs ownership. Evaluation cases age, integrations change and policies move. The firm should keep only the shared capability it can operate and buy specialist support where intermittent depth is more economic.' },
  ...
  'measure-automation-value': [{
    ...
    purpose: 'Strengthen the counterfactual and prevent unlike value claims from collapsing into one score.',
...
export const advancedCaseExtensionSections: Record<string, AddedSection[]> = {
  'cold-chain': [{
    ...
    purpose: 'Define what closure must prove and how later investigation can reconstruct it.',
  ...
  'property-pipeline': [{
    ...
    purpose: 'Show how the workspace handles transactions that cannot follow the normal gate sequence.',
    ...
    paragraphs: [
      { text: 'Exceptions should be typed as missing evidence, disputed evidence, external delay, policy waiver, identity conflict or system failure. Each type has an owner, due condition and escalation. A free-text note alone cannot support portfolio analysis or a reliable recovery route.' },
      ...
      { text: 'The operating owner should decide how long inactive and completed records remain searchable, which roles can reopen them and how legal or commercial holds apply. Lifecycle controls prevent a useful live workspace from becoming an unmanaged archive of sensitive transaction material.' },
    ],
  }],
  'professional-services-intake': [{
    heading: 'Matter acceptance and downstream handover',
    purpose: 'Connect the intake decision to the first accountable unit of professional work.',
    ...
    paragraphs: [
      ...
      { text: 'Conditional acceptance needs explicit actions and owners. A matter may open subject to engagement terms, additional identity evidence or specialist review. The state remains conditional until those actions complete, and downstream workflows can see the restriction.' },
  ...
  'field-service-planning': [{
    ...
    purpose: 'Turn actual field outcomes into governed updates to estimates, constraints and objectives.',
