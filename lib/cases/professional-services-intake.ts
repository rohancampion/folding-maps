...
export const study: CaseStudy = {
  ...
  title: 'Faster intake with professional control',
  summary: 'A specialist advisory firm is reducing repeated questions at intake while keeping every acceptance decision with a qualified professional.',
  status: 'Anonymised',
  brief: 'Professional confidentiality keeps the firm unnamed. The first-pass sample has not been supplied, so this case reports no completeness, time or acceptance result. The intervention asks for required facts early, runs fixed checks and prepares one clear file for professional review.',
  metrics: [
    { value: '5', label: 'required intake items', detail: 'Defined with the firm' },
    { value: '4', label: 'mandatory checks', detail: 'Identity, conflict, funding and eligibility' },
    { value: '1', label: 'professional decision', detail: 'Accept, decline or hold' },
  ],
  phases: [
    { label: 'Measure', detail: 'Count missing items and repeated questions in one service line.' },
    { label: 'Ask', detail: 'Request only the facts needed for the next decision.' },
    { label: 'Prepare', detail: 'Complete fixed checks and assemble a clear review file.' },
    { label: 'Decide', detail: 'A qualified professional accepts, declines or holds the matter.' },
  ],
  code: {
    title: 'Referral to professional decision',
    lines: ['referral = receive(payload)', 'missing = requirements.check(referral)', 'answers = request(missing)', 'checks = policy.run(answers)', 'decision = professional.review(referral, answers, checks)'],
    nodes: ['Referral', 'Missing-item check', 'Focused request', 'Mandatory checks', 'Professional decision'],
  },
  nextSteps: ['Complete the first-pass baseline', 'Approve collection and retention rules', 'Test the four mandatory checks', 'Pilot one service line with weekly professional review'],
};
...
export const editorial: CaseEditorial = {
  ...
  thesis: 'Urgent referrals arrive without the facts needed for conflict, identity, funding and eligibility checks. Senior staff then spend time reconstructing the intake before they can make a professional decision. The intervention requests missing items early, completes fixed checks and presents one clear file to the qualified person who decides.',
  sceneLabel: 'A representative referral',
  openingTitle: 'A valuable referral arrives before the firm can decide',
  openingParagraphs: [
  ...
  centralQuestion: 'Can the firm reach a sound professional decision sooner while protecting confidentiality and keeping every mandatory check clear?',
  sections: [
    {
