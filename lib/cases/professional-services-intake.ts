...
export const editorial: CaseEditorial = {
  ...
  openingParagraphs: [
    ...
    'The firm wants the work. It cannot say yes until it knows who the client is, who the other side is, who funds the work and which of its service routes the matter belongs on. Those four answers are administrative. The person assembling them is usually the most expensive person free that afternoon.',
  ],
  centralQuestion: 'The design succeeds only if the same referral reaches a recorded acceptance or a recorded decline sooner, with every mandatory test running on the same facts each time and the reasoning still attributable to a named professional.',
  ...
  sections: [
    {
      ...
      paragraphs: [
        { text: 'Intake looks like a judgement call and behaves like a collection problem. The judgement at the end of it is difficult and brief: a qualified person reads an assembled file and decides whether the firm can act, on what terms and for whom. Almost everything before that moment is the gathering of facts the firm already knew it would need. The engagement therefore starts by measuring how the time between a referral landing and a decision being recorded divides between the two.' },
        { text: 'The firm is counting the first pass. Each referral reaching one service line is read exactly as it arrived, before anyone has asked a follow-up question, and marked against the five items the mandatory tests require: identity evidence for the prospective client, every counterparty named in full, the documents the referral refers to, a stated basis for who pays, and scope specific enough to route the matter to a service. The number the firm needs is how many referrals arrive with all five, and nobody there can currently say what it is.' },
    ...
    },
    {
      ...
      paragraphs: [
        ...
        { text: 'The count runs twice. The first pass through one service line establishes what completeness on arrival currently looks like, and it should run before any software is built, because it decides whether the problem is the one described here. The same reading repeated after release, against the same five items, gives the only directly comparable figure this work will produce. Four measures attach to it: elapsed time from referral to recorded decision, the number of return passes a matter needs before it is complete, declines and holds broken down by the test that produced them, and referrer response to the request. Stop conditions are set in advance, among them a test returning different results on the same facts, a referral source going quiet, and an accepted file whose summary misstates the referral it came from.' },
        { text: 'The record outlasts everything else here. For every matter the firm took and every one it turned away, it should be able to show which facts it held, which test produced the answer and who put their name to it. The record should exist whether or not any drafting is ever automated, and it is the thing the regulator’s file reviews keep finding absent. Assisted summarising stays while it shortens assembly and leaves the tests untouched, and it goes the moment a second reader finds a summary asserting more than the referral supports. The gates hold their position either way.' },
      ],
    },
  ],
};
