...
export const study: CaseStudy = {
    ...
    summary: 'An operator whose temperature alerts arrive within a minute, reach nobody in particular, and record no state beyond sent.',
    status: 'Anonymised',
...
export const editorial: CaseEditorial = {
  statusStatement: 'The operator has asked not to be named. Discovery is still reading the alert extract, so this page shows no count from it and no chart. Every service level and expected benefit is a design target agreed for the work, and none should be read as an audited outcome.',
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        { text: 'Four categories divide the alerts. An alert is complete when the record holds an inspection, a disposition and the temperature that followed. It is accounted for when a maintenance or defrost entry explains the reading while no response of its own was written down. It is recoverable when nothing was written but a colleague can still say what happened. It is dark when neither the record nor anybody available connects it to an action.' },
        { text: 'Those categories describe the record and stop short of describing the response. Some dark alerts will have been handled by somebody who saw the message, walked to the room and went back to work. Counting them matters for that reason: the site cannot separate that night from the one where the message was never opened, and neither can an auditor asking about a specific date six months later.' },
        { text: 'The classification will settle nothing about product safety. It offers no view on whether the thresholds are correct, whether the probes hold their calibration, or whether any alert in the window put stock at risk. It can settle the order of the work, by showing how much of the response this site is able to evidence at all. No share from that reading is published here, because the extract has not been read to the end.' },
