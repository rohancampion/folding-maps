...
export const study: CaseStudy = {
  ...
  metrics: [
    ...
    { value: '24/7', label: 'named rota coverage', detail: 'Pilot requirement' },
  ],
  phases: [
    ...
    { label: 'Assign', detail: 'Send each qualifying alert to the named role on duty.' },
    { label: 'Prove', detail: 'Compare missed events, response time and closure detail during the pilot.' },
  ],
  code: {
    title: 'Alert to named response',
    ...
    nodes: ['Temperature alert', 'Signal check', 'Priority decision', 'Named person', 'Completed response'],
  },
...
export const editorial: CaseEditorial = {
  ...
  sections: [
    {
      heading: 'An alert without a named responder leaves risk open',
      paragraphs: [
    ...
    },
    {
      ...
      paragraphs: [
        { text: 'The operator has agreed the purpose, one-site scope, named response role and proposed fifteen-minute acknowledgement target. Discovery has also defined the alert review that will provide the starting point. The team has not completed that review or tested the service with a live shift.' },
    ...
    },
    {
      ...
      paragraphs: [
        { text: 'Measures include signal availability, faulty-signal detection, important events found only by current practice, false alarms per shift, time to a named person, oldest untouched case, completion quality and staff time per case. The operator will set acceptable limits before the pilot starts.' },
