...
export const study: CaseStudy = {
    ...
    code: {
      title: 'One append-only history stands behind every derived state',
    ...
    nextSteps: ['Agree the project sample and take the count before the release ships', 'Release the shared record with capture inside existing tools', 'Review every open commitment without a named colleague weekly', 'Decide on connected documents only after the second count'],
};
...
export const editorial: CaseEditorial = {
  ...
  thesis: 'The cost here is reconstruction. Answering one customer’s question means opening several records and asking somebody. The first release should therefore be designed so that the shared record is a by-product of work already being done, and it should be judged by a count of that assembly taken before the release ships and again once it has been in use.',
  ...
  centralQuestion: 'A shared record justifies itself here on two counts: whether it removes the lookup a colleague performs today, and whether it can stay current without anyone typing the same fact a second time.',
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        { text: 'Published evidence on systems of this kind is unusable in either direction. CIO, reviewing a dozen frequently quoted analyst estimates of how often customer-relationship projects fail, found the numbers ranging from 18 to 69 percent depending on how each analyst defined failure, and put the average at about a third. A spread that wide says nothing about any particular project, and the firms quoting the top of it are usually selling the remedy.', sources: [4] },
        { text: 'A sharper objection is local. In a business this size the several places may matter less than the argument assumes, because the people are good and the context lives in their heads. A colleague who has worked on the same boats for a decade may give a better answer from memory than a record gives from a screen, and the assembly cost buys accuracy. Estimates that a searchable record cuts information-search time substantially, including the figure of up to 35 percent modelled by the McKinsey Global Institute in 2012, are projections from a period before this software existed in its present form. None of them was measured in a boatyard. The version of this objection with real force concerns who absorbs the cost. Each assembly pulls a colleague off a job, so the delay lands inside the yard while the customer experiences a prompt reply.', sources: [2] },
    ...
    },
    {
      ...
      paragraphs: [
        { text: 'Discovery has produced an architecture and a measurement specification, and it has produced no result. The engagement holds a definition of the count, an identity model, an event history, a rule for deriving project state and a boundary around what the first release will do. Everything on the benefit side remains a design target.' },
...
export const research: ResearchFinding[] = [
  { statistic: '1,200 a day', finding: 'Instrumented workers switched between applications and windows about 1,200 times a day, losing just under four hours a week to reorientation', implication: 'The cost of assembling an answer from several places is ordinary enough to stay invisible in any budget, and an invisible cost is rarely designed out.', source: 'Harvard Business Review, How Much Time and Energy Do We Waste Toggling Between Applications?, 2022', href: 'https://hbr.org/2022/08/how-much-time-and-energy-do-we-waste-toggling-between-applications' },
