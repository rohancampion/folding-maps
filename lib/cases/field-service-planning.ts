...
export const editorial: CaseEditorial = {
  ...
  centralQuestion: 'This service is worth building if a dispatcher can see, for every permitted way of absorbing an urgent job, which existing commitments it moves and which of those customers have already been moved this week.',
  ...
  sections: [
    {
      ...
      paragraphs: [
        { text: 'Discovery opens by specifying a count the operator has never run. Six consecutive weeks of work orders from a single region, each job read for the conditions that must hold before an engineer can be sent to it at all. A named certification. An appointment window agreed with the customer. A part that no van carries as standard. All three fields already sit in the operator’s work-order records. Nobody has put them together and asked how much of an ordinary week they describe, and until somebody does, any claim about where planning effort should go is a claim about a distribution nobody has seen.' },
        { text: 'Two things turn on that count, and they pull in opposite directions. Where a job carries a certification requirement and a customer appointment window together, the engineers who could lawfully and practically take the work are a small fraction of the roster on shift, and distance enters the problem only after that reduction has happened. Where a job carries neither, the whole roster is available and something else has to choose between them. Dispatchers at this operator describe the first case as the common one. That is a recollection offered in interviews, and the design should be built so that the count is free to contradict it.' },
    ...
    },
    {
      ...
      paragraphs: [
        { text: 'Quiet Gears entered this engagement carrying a scoring model of its own. It ranked candidate assignments on four preference weights covering service level, operational priority, travel efficiency and plan stability, each printed to the nearest percentage point. An earlier version of this page published those weights. They were the firm’s own view of what a dispatcher should care about, and no part of them had been calibrated against the operator’s data or against any dispatcher’s revealed choices.' },
        { text: 'Applying a condition filter before any weighting shows where such weights could bite at all. On a job carrying both a certification requirement and an appointment window, so few assignments survive the filter that the ranking among them rarely turns on which weighting is used. On a job carrying neither, the weights would govern the answer completely, and there the design was asking dispatchers to accept a preference nobody had measured. Neither case justifies publishing a percentage. The count will establish how much of the week falls into each case, which changes the size of the problem and leaves the verdict on the weights where it already stood.' },
        { text: 'So the weights were withdrawn. What replaced them is a filter and a statement. The filter removes every assignment that breaches a certification, working-hours or parts condition, and reports which condition removed it. The statement describes, for each surviving option, exactly which existing commitments would move and by how long. The dispatcher then chooses between options that are all permitted, on a description of consequences that carries no hidden coefficient.' },
        { text: 'This defect is worth naming, because planning software is full of it. A number printed to a percentage point tells a reader that somebody measured a preference and found it to be that. Where the measurement never happened, the precision does rhetorical work the analysis cannot support, and a footnote conceding as much removes none of the impression the chart has already made. The same test applies to a chart of operational counts, which is why this page carries none until the extract arrives.' },
      ],
      exhibits: [{ kind: 'process', afterParagraph: 2 }],
    },
    {
      heading: 'Displacement is the figure dispatch actually needs',
      ...
      paragraphs: [
        ...
        { text: 'Four components carry the work. Work orders and roster records pass a validation step at the boundary, because a missing duration estimate makes a day appear to fit when it does not. A condition filter reduces the candidate assignments and records its reasons. A displacement model describes each survivor in moved commitments. A dispatcher console presents them and captures the decision. NCSC guidance on operational technology supports validating those inputs before they reach the planner.', sources: [4] },
