  ...
  sections: [
    {
      ...
      paragraphs: [
        ...
        { text: 'English courts have already tested what happens when that inspection is skipped. In Ayinde v London Borough of Haringey the claimant’s grounds cited five decisions that do not exist, one of them attributed to the Court of Appeal. In Al-Haroun v Qatar National Bank, eighteen of the forty-five authorities placed before the court were fabricated. The Divisional Court declined to begin contempt proceedings and referred the lawyers to their regulators, holding that a practitioner who uses generative tools for research carries a professional duty to check the result against authoritative sources before relying on it.', sources: [ayindeJudgment] },
        ...
        { text: 'Treating the note as a single artefact leaves a reviewer two options, both bad: trust the whole document or reproduce the whole document. Treating it as a set of propositions changes that arithmetic. Each material claim carries the question that produced it, the passage retrieved, the issuing court or body, the effective date, the jurisdiction and the version of the workflow that assembled it. A bad claim can then be struck without discarding the sound work around it, and the reasons for striking it become countable across matters.' },
      ],
    ...
    },
    {
      ...
      paragraphs: [
        { text: 'Retrieval should run against an approved list and nothing wider. A matter workspace opens with a manifest that names the matter documents, the internal precedent and know-how cleared for reuse, and the external collections the firm licenses, each carrying its jurisdiction, its effective period and the people entitled to see it. Anything absent from the manifest is unavailable to the model, including material the firm holds elsewhere. An unrestricted pool assembled because it was convenient to assemble is the condition the manifest exists to prevent.' },
    ...
    },
    {
      ...
      paragraphs: [
        { text: 'A source-grounded interface can make weak work look safer. Citations displayed beside every sentence signal that checking has already happened, and a long run of outputs that turn out to be sound is precisely the condition under which a reviewer stops opening sources. The rare consequential error then reaches a reader whom the interface has trained to expect none. Detailed provenance also costs money to build and to keep current, and that cost falls on the firm whether or not a single reviewer clicks through to a passage.' },
        { text: 'The economic case against proposition-level review is stronger than the firm would prefer. A benchmark reported in October 2025 scored three legal research systems and one general model against a lawyer control group on research questions, and the lawyer baseline finished last at 69 percent while the tools sat between 74 and 78 percent. An earlier round of the same benchmark placed assisted tools ahead of the lawyer group on four of seven document tasks. If assisted research is already more accurate on average than the professional checking it, a control built around professional scepticism is buying reduced variance at a real price in time.', sources: [valsBenchmark] },
