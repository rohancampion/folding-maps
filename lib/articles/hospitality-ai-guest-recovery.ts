...
export const article: NewsEditorial = {
  ...
  sceneTitle: 'Quarter past one, and the allocated room is unusable',
  sceneParagraphs: [
    'The guest reaches the desk at quarter past one after a diverted flight. The reservation is confirmed and paid, the property management system shows the room allocated, and a maintenance note taken in the early evening marks it out of service. The loyalty record sits under an older email address, so the profile on screen shows no status at all. One colleague is on duty. The duty manager is asleep on site and can be woken.',
  ...
  sections: [
    {
      ...
      paragraphs: [
        ...
        { text: 'Written down, the sequence looks like a specification, and the specification is the part of this subject that has been published most often. It is also the part nobody argues with. Every operator agrees that identity should resolve correctly and that a colleague should know the limit of their authority. Agreement at that level costs nothing and decides nothing, because the question a property faces is whether building it changes an outcome the experienced colleague was already producing.' },
    ...
    },
    {
      ...
      paragraphs: [
        ...
        { text: 'Churn cuts the other way as well. A property replacing most of its front-of-house staff each year will struggle to keep any system accurate, and a recovery record is worth no more than the maintenance notes and room states feeding it. Few people train hard on a tool they expect to use for a single season. Override reasons go unrecorded, connectors drift, and the record becomes another screen that has to be checked against a phone call. Published examples of connected guest data come from groups such as Radisson with central engineering teams, described in the vendor account, and a four-property operator has none of that.', sources: [googleRadisson] },
        { text: 'Those objections narrow the case and they do not close it. Handling cost and avoided second failures are smaller prizes than loyalty, and they have the advantage of being measurable at one property in one quarter, which loyalty has never been. The maintenance risk is real and it argues for connecting less, so the number of things that can drift stays small enough for one identified person to check on a Monday morning. The sharpest form of the objection is that the same budget would buy retention, and a property that held on to its people for another year would need less of this. That is probably true. It is also unavailable to most operators at the wage the sector currently pays, and the connected record is what remains once the retention argument has been made and lost.' },
      ],
