...
export const article: NewsEditorial = {
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        { text: 'One task accepted into operations costs more than an inference call. Add retrieval and orchestration. Add the attempts that failed and had to be retried. Add the share of work a person finished by hand, and the review effort spent checking the share a person did not. Add evaluation, monitoring, and the engineering that falls due again every time a prompt, a provider or a model version changes.' },
    ...
    },
    {
      ...
      paragraphs: [
        ...
        { text: 'The concession has a boundary. The buyer must keep the evaluation set, the rubric, the acceptance thresholds and the business rules, because those are what make a second supplier assessable at all. Execution can sit inside proprietary features. The cost of leaving should be calculated before signing, written down and revisited at renewal, so that the dependency remains a decision the firm has taken at a known price.' },
      ],
    },
    {
      heading: 'Set one denominator before the next model switch',
      role: 'conclusion',
      transition: 'Because the managed route and the self-hosted route can each be defended independently, the choice has to be settled by a measure that applies to both.',
      paragraphs: [
