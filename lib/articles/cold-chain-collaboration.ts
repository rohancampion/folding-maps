  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        { text: 'With validation in place, the distinction that carries the whole design becomes affordable. An alert records that a rule fired. A case carries the validated reading and its duration, the state of the asset, the product and location, the policy that applies, the colleague it is assigned to, the action taken and the evidence that temperature recovered. Whatever is still missing stays visible on the face of the case. What reaches the reviewer is a decision object.' },
        { text: 'Two of those fields do the real work: the named responder and the stated time to respond. Food Standards Agency guidance pairs monitoring with effective corrective action and leaves the judgement with the business, which means an unassigned excursion satisfies nothing at all. Closure should record what was inspected, what was moved or destroyed and what temperature did afterwards, so that the assurance file becomes a by-product of the response.', sources: [foodStandards] },
      ],
    ...
    },
    {
      ...
      paragraphs: [
        { text: 'What follows from all of this is a narrow proposal. Take twelve months of existing telemetry, maintenance records and shift notes for one asset class at one site, and replay them through the proposed policy in observation mode, with nothing reaching a phone. The replay answers three things worth knowing before money is committed: how many cases the policy would have raised per shift, which recorded incidents it would have missed, and how many of its cases would have closed as instrumentation faults.' },
        { text: 'Observation mode also prices the dark stretches. Where the replay raises a case that the available data cannot resolve, that case closes as unresolvable, and the count of unresolvable cases is the argument for extending coverage, expressed in the unit an operation cares about, which is decisions it could not make. That count is worth more than a sensor quotation, because it comes from events the site actually had. The same replay shows which of the current alerts nobody acted on, and that list is usually cheaper to fix than the sensor estate.' },
        { text: 'Comparison with current practice should use measures a duty manager recognises: the share of the path under valid signal, the proportion of cases closing as instrumentation faults, the age of the oldest unassigned case, the time from breach to first human action, and the effort now spent assembling evidence for an audit. Any target set against those measures stays a hypothesis until the replay and a live parallel run have produced numbers of their own.' },
        { text: 'None of this moves the decision away from the operator. Judgement about whether product is safe stays with the people accountable for it, and a monitoring service implying otherwise would be a liability to the business that bought it. What the design offers is a faster and more complete account of what happened, assembled while the eight minutes still matter, and a record afterwards that somebody can defend.' },
      ],
    },
  ],
};
...
export const evidenceViews: EvidenceView[] = [
  ...
  },
  {
    ...
    points: [
      ...
      { label: 'Ran above the manageable band', value: 42, display: 'The remainder', detail: 'Derived by subtracting the two published shares, so roughly two consoles in five carried a rate the guide treats as unworkable.' },
