...
export const article: NewsEditorial = {
  ...
  sceneParagraphs: [
    ...
    'The arithmetic omits everything between the task and the ledger. Part of the volume never reached the new route. Some outputs came back for correction. The time genuinely released landed in whatever the team did next, which nobody was measuring. A year on, the operating cost is unchanged and the productivity claim is still on the slide.',
  ],
  sections: [
    {
      ...
      paragraphs: [
        ...
        { text: 'One moderator transfers between the two studies. Assistance pays most where the worker lacks context the model can supply, and pays least, or costs time, where the worker already holds more context than the model can reconstruct. METR published a second reading of the experiment in February 2026, reporting a smaller and statistically inconclusive slowdown among newly recruited developers. That is the direction the moderator predicts.', sources: [metrStudy, metrRerun] },
    ...
    },
    {
      ...
      paragraphs: [
        ...
        { text: 'Redeployment decides whether anything financial happens at all. Released capacity absorbed into general availability leaves no trace in any account, and it is indistinguishable from a benefit that never existed. Capacity pointed at an identified queue, an identified backlog or a piece of demand the business already counts can be observed moving. The ledger structure below shows which reading supports each stage and where the confidence level attaches.' },
        { text: 'Recording the bridge honestly usually reduces the headline number by a large fraction. A programme reporting a modelled twenty-hour cash equivalent against a hundred theoretical hours is in a defensible position, because it can say where the other eighty went and what would have to change to recover any of them. A committee can interrogate that position. It can ask why adoption stalled short of eligible volume, whether the review step is permanent or an artefact of an immature model, and which queue the redeployed capacity was meant to reach. None of those questions is available against a single multiplied figure.' },
      ],
    ...
    },
    {
      ...
      paragraphs: [
        ...
        { text: 'The concession the method has to make is about pricing, and about how confidence is recorded. A benefit may sit in the ledger with an identified mechanism, an identified manager and a stated result that would disprove it, at low confidence and with no pound attached. The programme in the opening arithmetic reported as cash a quantity it had never observed. That is the defect, and it survives whatever the true benefit turns out to be.' },
      ],
    },
    {
      heading: 'Report confidence alongside every figure in the ledger',
      role: 'conclusion',
      transition: 'Since a cash test alone would reject benefits the firm has good reason to buy, the ledger has to carry claims it cannot yet price and say plainly that it cannot.',
      paragraphs: [
        { text: 'Restate the hundred hours as a hypothesis with a date on it. The entry should hold the baseline period and eligible volume, the observed share completed through the new route, the effort spent on review and exceptions, the identified destination of any released capacity, and a confidence level that starts low and moves only as observation replaces assumption. Confidence is the field that does the work, because the next reading exposes any inflation. It also fixes the reporting cadence, since a claim held at low confidence has to state the reading that would raise it and the date by which that reading will exist.' },
        { text: 'Two disciplines keep the ledger honest at low cost. Measure the workflow in front of you, since the research shows the effect size belongs to the worker and the task and travels badly between them. Then separate the operating change from the financial one at the point of entry, so that an improvement in handling time is never promoted into a saving nobody has found.' },
        { text: 'Measurement repays its cost when it changes a decision, and the decision it should change most often is how much to claim. A programme that reports a range with its causal chain visible, and marks the stages it has not yet observed, gives an investment committee something it can act on. A precise figure with no route to the accounts gives it something to repeat.' },
      ],
    },
  ],
};
...
export const evidenceViews: EvidenceView[] = [
  {
    ...
    interpretation: {
      ...
      management: 'Measure the target workflow against a separate baseline, adoption share, review effort and exception cost.',
    },
    ...
    points: [
      ...
      { label: 'Maintainers on internal repositories', value: 81, display: '-19%', detail: 'Completion time worsened for experienced developers working in codebases they knew well.' },
    ],
  },
  {
    label: 'Value bridge',
    ...
    points: [
      ...
      { label: 'Completed through the new route', value: 82, display: '82 hours', detail: 'Eligible volume that travelled the new path, which the workflow can count directly.' },
      ...
      { label: 'Redeployed to counted work', value: 48, display: '48 hours', detail: 'Capacity pointed at an identified queue or backlog whose output is already measured.' },
