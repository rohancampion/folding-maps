...
export const article: NewsEditorial = {
  ...
  standfirst: 'Model capability varies from one task to the adjacent one, and nothing in an agent’s output marks the boundary. Fixed rules, a bounded model and an identified person on exceptions put that boundary where management can see it.',
  thesis: 'Authority over live work should be granted in tiers by consequence, and only once the fixed parts of the workflow run deterministically and every known exception routes to an identified person.',
  ...
  sections: [
    ...
    },
    {
      ...
      role: 'conclusion',
      transition: 'Because early autonomy is defensible only where review is more than a click, the release decision turns on the evidence produced for exceptions.',
      paragraphs: [
